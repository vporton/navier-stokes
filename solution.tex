...
As shown in \cite[section~12, p.~11]{limit}, on $\SUPER(X)$ there is defined funcoid structure, what allows to take limits on it.
...
\begin{thm}
If a set $X$ is Lebesgue measurable and $f:X\to\SUPER(\mathbb{R}^N)$ is continuous, then:
\begin{enumerate}
...
Therefore the integral~$u$ is $C^\infty$ (on the set $\SUPER(\mathbb{R}^N)$).
