...
Note that \[\SUPER(X)=X\cup\operatorname{SLA}(X)\cup\operatorname{SLA}(\operatorname{SLA}(X))\cup\dots\] where $\operatorname{SLA}(X)\supseteq X$ and all generalized limits of functions to~$X$ fall into $\operatorname{SLA}(X)$.
...
\begin{defn}
Let $a,b\in\operatorname{SLA}(X)$. Then for limits of functions $Y\to X$:
