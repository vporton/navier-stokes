...
\begin{proof}
\begin{grayed}
Suppose it does not hold. Then?? there are sequences~$a$ and~$b$, $a-b\to 0$ such that $f(a_n)\neq kf(b_n)$ for all big~$n$ and all $k\in\mathbb{R}$.

But due to continuity of $f$ there is a $\lim f(a_n) = \lim f(b_n)$. Thus for all big~$n$ $f(\lim_{n\to\infty}a_n) = f(\lim_{n\to\infty}b_n)$.
\end{grayed}
...
\[ \inf_{k_a,k_b\in\mathbb{R}}|k_a a-k_b b| > 0. \]
\end{proof}
