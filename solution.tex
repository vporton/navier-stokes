...
Extraordinary claims need extraordinary evidence. The claim is a partial Millennium Prize Problem solution. The evidence of this ``functional extension'' theorem is an extraordinary evidence, because it is a new very powerful theorem in general topology and its proof is extraordinary longer than a noob would expect.
...
We are going to prove existence (in the sense of classical real-number solutions) and smoothness~($C^\infty$) of solutions to Navier-Stokes equations on $\mathbb{R}^3$.
...
\begin{thm}
For each $S\in\subsets\SUPER(\mathbb{R})$, such that $\forall a\in S:a>B$ for some constant $B\in\mathbb{R}$, there exists $\inf S\in\SUPER(\mathbb{R})$.
\end{thm}
...
\begin{proof}
Obvious? (There is a proof by ChatGPT. Below it's also proved more general case of derivatives.)
\end{proof}
