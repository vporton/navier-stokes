...
For antisymmetry, let $a\leq b$ and $b\leq a$. We have $\lim_{x\to 0}|g(x)-f(x)|\geq 0$ and $\lim_{x\to 0}|g(x)-f(x)|\leq 0$. So, $\lim_{x\to 0}|g(x)-f(x)|=0$ and therefore $a=b$.\fxnote{More detailed proof.}
\end{proof}
...
Define the Picard iteration mapping~$\mathcal{T}$ on time-dependent vector fields by
\begin{equation}
...
\begin{thm}
The heat semigroup $G_t$ is infinitely smoothing, that is $G_t f\in C^\infty$ for any $f:X\to L^1(\SUPER(\mathbb{R}))$ for $t>0$. (The norm is taken in~$\SUPER(\mathbb{R})$\fxnote{Mention that $\mathbb{P}$ is bounded to prove $L^1$.})
\end{thm}
