...
We will consider functions $\mathbb{R}^P\to\mathbb{R}^Q$ or sequences $\mathbb{N}\to\mathbb{R}^Q$ for some $P,Q\in\mathbb{N}$. We define \emph{generalized limits} of such functions  (and of sequences at infinity) by the formula~(1) in~\cite[p.~6]{limit}:
\begin{itemize}
\item For the group~$G$ in~\cite{limit} take the group of translations of $\mathbb{R}^P$ (or, alternatively, its isometries) or $\mathbb{Z}$.
...
It's easy to show that for this relation, if considered as a funcoid~\cite{volume-1-edition1}, $\mathrm{X}^{-1}\circ\mathrm{X}=\mathrm{X}$.
\item It's obvious that $\mathrm{X}$ (as a funcoid~\cite[chapter~7, p.~144]{volume-1-edition1}) commutes with elements of~$G$.
\item Thus generalized limit is defined.
\end{itemize}

The generalized limit is defined for every function at all points.

In~\cite{limit} the set of all generalized limits over $X$ we denote as $\SLA(X)$. We can assume $X\subseteq\SLA(X)$ (by injectivity of $\tau$~\cite[p.~7]{limit}, using that $\mathbb{R}^Q$ is Hausdorff). When the usual limit exists, it is equal to the generalized limit~\cite[corollary 3, p.~7]{limit}.

We have all derivatives on the set~$X$ to be always defined taking values in~$\SLA(X)$.

Let denote
\[ \SUPER(X) = X\cup\SLA(X)\cup\SLA(\SLA(X))\cup\dots. \]
We have $\SUPER(X)$ closed regarding derivatives.

We have all continuous finitary operations on~$\mathbb{R}^Q$ extendable to~$\SUPER(\mathbb{R}^Q)$ \cite[section 9, p.~8]{limit}. For all such operations, algebraic identities ($f(\dots)=g(\dots)$) on~$\SUPER(X)$ are the same as on~$X$~\cite[theorem 6, p.~10]{limit}, particularly generalized limit is a linear extension of the usual limit.

I will call functions with values in $\SUPER(X)$ ``su\-per-sin\-gu\-lar functions''.
...
\begin{defn}
Let $a,b\in\SLA(X)$. Then for limits of functions $Y\to X$:
...
\begin{proof}
Let
\[ a,b\in\underbrace{\SLA(\dots\SLA(\mathbb{R})\dots)}_{\text{Apply $\SLA$ $K$ times}}. \]
...
\begin{thm}
For each $S\in\subsets\underbrace{\SLA(\dots\SLA(\mathbb{R})\dots)}_{\text{Finite number of times}}$, such that $\forall a\in S:a\ge B$ for some constant $B\in\mathbb{R}$, there exists $\inf S\in\SUPER(\mathbb{R})$.
\end{thm}
...
For $\SLA(\dots\SLA(\mathbb{R})\dots)$ (finite number of limits) the same reasoning applies inductively.
\end{proof}
...
\begin{thm}
$C^\infty$ supersingular mild solutions of Na\-vi\-er-Sto\-kes equations are exactly $C^\infty$ solutions of Na\-vi\-er-Sto\-kes equations (without the requirement that~$p$ is $C^\infty$) taken on the space of supersingularities.
\end{thm}
...
It is well-known that~$u$ is classical for some time interval starting from $t=0$. Suppose that at the time $t_{\operatorname{crit}}$, the solution becomes non-clas\-si\-cal. This is possible only at a border of a ``classical'' zone (the integrand is equal to~$0$), therefore the left derivative (and by $C^\infty$, right derivative, too) of the integral is in~$\mathbb{R}$. This makes the change of the left part of the equation ``gradual'' (derivative is in~$\mathbb{R}$), what contradicts non-clas\-si\-ca\-li\-ty of $G_t u(0) - u(t)$ and therefore of $u(t)$.
