...
\[
G_t u(0) - u(t) = \int_0^t \bigl(G_{t-s} \mathbb{P}\nabla\cdot(u\otimes u)(s) - \mathbb{P}f(s)\bigr)ds.
\]

It is well-known that~$u$ is classical for some time interval starting from $t=0$. Therefore, there is a leftmost zone~$Z$ for the integral argument. If this is the only zone, we are done: the classical solution is this. Otherwise, the zone has a leftmost non-ze\-ro border point $t_B$ (take infimum of all its border points, and prove that this $t_B$ is a border point by continuity of the function under the integral), and therefore $u(t_B)=0$, and we also know that the further in time solutions are classical.
