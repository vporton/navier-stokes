...
It is well-known that~$u$ is classical for some time interval starting from $t=0$. Therefore, there is a leftmost zone~$Z$ for the integral argument. If this is the only zone, we are done: the classical solution is this. If the zone has an end time, at the end time of the zone $u=0$, and we also know that the further in time solutions are classical.
