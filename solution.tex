...
For antisymmetry, let $a\leq b$ and $b\leq a$. %Then there are $f',g':X\to\mathbb{R}$ such that $\lim_{x\to 0}f'(x)=a$ and $\lim_{x\to 0}f''(x)=a$ and $f'\leq g'$ and $f'',g'':X\to\mathbb{R}$ such that $\lim_{x\to 0}g'(x)=b$ and $\lim_{x\to 0}g''(x)=b$. and $f''\geq g''$. 

For $a\leq b$ we have $\lim_{x\to 0}|g(x)-f(x)|\geq 0$ and $\lim_{x\to 0}|g(x)-f(x)|\leq 0$. So, $\lim_{x\to 0}|g(x)-f(x)|=0$ and therefore $a=b$.\fxnote{More detailed proof.}
\end{proof}

\begin{thm}
For each $S\in\subsets\SUPER(\mathbb{R})$, such that $\forall a\in S:a>B$ for some coonstant $B\in\mathbb{R}$, there exists $\inf S\in\SUPER(\mathbb{R})$.
\end{thm}
...
\[
\mathcal{R}_j f(x) = c \lim_{\epsilon\to 0}\int_{\mathbb{R}^3\setminus B_{\epsilon(0)}}\frac{(x_j-t_j)f(t)}{|x-t|^4} dt.
\]
...
Because $u$ is continuous, we can split this equality into zones, and it is true in every zone.
On borders of zones, it can be constructed as a limit of the Poisson equation's solution in a zone. It does not depend on choice of the zone we take limit in, because~$u\in C^\infty$.
