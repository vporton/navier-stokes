...
\title[3D Navier-Stokes]{A Proof of Existence of a Smooth Classical Solution of 3D Navier-Stokes Equations}
...
\begin{proof}
To prove that $f(x)=0$ on boundaries of zones, consider two only possible cases for $x\in X\cap\partial Z_k$:
\begin{enumerate}
\item ``$x\in\overline{Z_k}$ and $\forall\epsilon>0:\exists a\in\mathbb{R}:(\abs{x-a}<\epsilon\land f(a)=0)$'' ??

\item ``$x\in\overline{Z_k}$ and $\forall\epsilon>0:\exists a\in\mathbb{R}:(\abs{x-a}<\epsilon\land f(a)=k'g(a))$, where $k'\in\SUPER(\mathbb{R})\setminus\{0\}$ and $g:X\cap\overline{Z'}\to\mathbb{R}$ where zone $Z'\ne Z_k$.'' ??
\end{enumerate}
