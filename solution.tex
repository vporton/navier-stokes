...
\begin{thm}
(The following does apply for $\mathbb{R}$-va\-lu\-ed functions defined on well-be\-ha\-ved subsets of $\mathbb{R}^M$. For details and more general cases see~\cite{limit}.)

The $\lim$ (limit) functional can be linearly extended to the space~$\SUPER(X)$ (where $X$ is the set non-ex\-ten\-ded limits take) of all functions and on the space of extended limits all continuous operations can be extended with preservation of all algebraic identities.

Moreover, for the limit extension it can hold, that any finite number of limits $\lim\dots\lim f$ (for $f:Y\to X$) is defined.
\end{thm}
