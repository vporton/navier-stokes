...
Extaordinary claims need extraordinary evidence. The claim is a partial Millennium Prize Problem solution. The evidence of this ``functional extension'' theorem is an extraordinary evidence, because it is a new very powerful theorem in general topology and its proof is extraordinary longer than a noob would expect.
...
\begin{thm}
The $\lim$ (limit) functional can be linearly extended to the space of all functions and that on the space of extended limits all continuous operations can be extended with preservation of all algebraic identities.
\end{thm}
...
\begin{enumerate}
\item ``$a\in\overline{Z_k}$ and $\forall\epsilon>0\exists x\in\mathbb{R}:(\abs{a-x}<\epsilon\land f(x)=0)$.'' This is obviously\fxnote{More detailed proof.} possible only if $f(a)=0$.
...
$f(x)$ belongs to the same zone $Z_k$, unless $f(a)=0$.
\end{enumerate}
