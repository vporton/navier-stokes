...
\begin{enumerate}
\item $\setcond{x\in X}{f(x)\ne 0}$ is partitioned into Lebesgue measurable sets (\emph{open zones}), each zone \[ Z=\setcond{ka}{k\in\SUPER(\mathbb{R}),k\ne 0} \] for some $a\in\SUPER(\mathbb{R})\setminus\{0\}$.
\item $f(x)=0$ for each $x$ on the boundary of a each zone.
\item $f(x)=k_Z g_Z(x)$ for each $x\in\overline{Z}$ for some $k_Z\in\SUPER(\mathbb{R})\setminus\{0\}$ and $g_Z:\overline{Z}\to\mathbb{R}$ for each zone $Z$.
\end{enumerate}
\end{thm}
