...
\fxerror{It is possible $0\notin X$.}

\begin{defn}
Let $a,b\in\SUPER(\mathbb{R})$. \[ a<b\eqdef a\ne b\land\exists f,g:X\to\mathbb{R}:\left(\lim_{x\to 0}f(x)=a\land\lim_{x\to 0}g(x)=b\land f<g\right). \] \fxnote{Define~$X$.}
\end{defn}
...
\begin{proof}
Transitivity and an\-ti-ref\-le\-xi\-vi\-ty are obvious. For antisymmetry, let $a<b$ and $b<a$. Then $a\ne b$ and ??
\end{proof}

\begin{thm}
For each $S\in\subsets\SUPER(\mathbb{R})$, such that \[ \forall a\in S\,\exists F:X\to\mathbb{R}:\left(\lim_{x \to 0}F=a\land F>0\right), \] there exists $\inf S\in\SUPER(\mathbb{R})$.
\end{thm}
