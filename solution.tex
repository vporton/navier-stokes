...
\begin{defn}
Let $a,b\in\SUPER(\mathbb{R})$. \[ a<b\eqdef a\ne b\land\exists f,g:X\to\mathbb{R}:\left(\lim_{x\to 0}f(x)=a\land\lim_{x\to 0}g(x)=b\land f<g\right). \]
\end{defn}

\begin{prop}
The above defines is a (strict) partial order on~$\SUPER(\mathbb{R})$.
\end{prop}

\begin{proof}
Transitivity and an\-ti-ref\-le\-xi\-vi\-ty are obvious.

\fxnote{Detailed proof.}For antisymmetry, let $a<b$. Then $a\ne b$ and there is an ultrafilter $\mathcal{F}$ in vicinity of $0$ such that both $\supfun{f'}\mathcal{F}<\supfun{g'}\mathcal{F}$ and $\supfun{g''}\mathcal{F}<\supfun{f''}\mathcal{F}$ for some $f',g',f'',g'':X\to\mathbb{R}$, $\lim_{x\to 0}f'(x)=f''(x)=a$ and $\lim_{x\to 0}g(x)=g''(x)=b$. This is impossible, because $\supfun{\SUPER(\mathbb{R})}\supfun{f'}\mathcal{F}>\supfun{\SUPER(\mathbb{R})}\supfun{g'}\mathcal{F}$ and therefore $\supfun{f'}\mathcal{F}-\supfun{g'}\mathcal{F}>0$ making it impossible $b<a$.
\end{proof}

\begin{thm}
For each $S\in\subsets\SUPER(\mathbb{R})$, such that $\forall a\in S:a>0$, there exists $\inf S\in\SUPER(\mathbb{R})$.
\end{thm}
...
Let
\[ m=\lim_{x\to 0}\left(x\mapsto\inf_{f\in F}\max\{f(x),0\}\right). \]
($\inf_{f\in F}\max\{f(x),0\}$ exists because this set is bounded from below.) We will show that $m$ is the infimum of~$S$.
...
\[ \setcond{\setcond{x\in X}{\exists a\in\mathbb{R}\setminus\{0\}:f(x)=ka}}{k\in\SUPER(\mathbb{R})\setminus\{0\}}. \]
\item $f(x)=0$ for each $x$ on the boundary $X\cap\partial Z$ of a each zone.
\item $f(x)=k_Z g_Z(x)$ for each $x\in X\cap\overline{Z}$ for some $k_Z\in\SUPER(\mathbb{R})\setminus\{0\}$ and $g_Z:X\cap\overline{Z}\to\mathbb{R}$ for each zone $Z$.
\end{enumerate}
\end{thm}

\begin{proof}
Really, the distance between $c(a)$ and $c(b)$\fxwarning{What is~$c$? Give full proof.} for $\forall p,q\in\mathbb{R}:pa\ne qb$ is
\[ \inf_{p,q\in\mathbb{R}}|pa-qb| > 0, \]
...
Define Lebesgue integrals on the space of continuous generalized limits functions as \[ \int_A f(x) dx = \sum_{Z\in S}k_Z\int_Z a_Z(x) dx \] if each Lebesque integral on the right side exists and the sum is absolutely convergent (and so does not depend on the order of summation), where $S$~is a set of zones.
...
Let's denote for $\underline{a}\mathcal{X}=\supfun{\mathbb{R}}\supfun{f}\mathcal{X}$ for $a\in\mathbb{R}$, $f\in X\to\mathbb{R}$, $\lim_{x\to\mathcal{X}}f(x)=a$.
...
So, $G_t f^{(n)}\in C^\infty$ for any classical~$f$.

\fxnote{Formulate the proof exactly.}Let $F$ be a supersingular function. Let $\mathcal{X}$ be an ultrafilter or, equilently, a sequence of points converging to a point $x$. $F\mathcal{X} = \lim_{x\to\mathcal{X}}f^{(n)}(x)$.

By the above shown interchange of limits,
\[ G_t F\mathcal{X} = G_t\lim_{x\to\mathcal{X}}f^{(n)}(x) = \lim_{x\to\mathcal{X}}G_t f^{(n)}(x). \]
