...
Refer to~\cite{limit} for definitions and theorems that warrant for our spaces ($\mathbb{R}^P$):

The $\lim$ (limit) functional can be linearly extended to the space all functions having as the range the space~$\SUPER(X)$ of generalized limits (where $X$ is the set non-ex\-ten\-ded limits take).

Limits can be futher taken (with values remaining in~$\SUPER(X)$) for functions with values in $\SUPER(X)$.

All finitary continuous operations can be extended from~$X$ to~$\SUPER(X)$ with preservation of all algebraic identities.

I will call functions with values in $\SUPER(X)$ ``su\-per-sin\-gu\-lar functions''.

Note that \[\SUPER(X)=X\cup\SLA(X)\cup\SLA(\SLA(X))\cup\dots\] where $\SLA(X)\supseteq X$ and all generalized limits of functions to~$X$ fall into $\SLA(X)$.

Note that the above allows to take (su\-per-sin\-gu\-lar) derivatives of arbitrary functions.
