...
It is well-known that~$u$ is classical for some time interval starting from $t=0$. Therefore, there is a leftmost zone~$Z$ for integral arguments. If this is the only zone and $f\equiv 0$, we are done: the classical solution is easy (divide by $k_Z$). If this is the only zone and $f\not\equiv 0$, we have a contradiction: $k_Z$ for the left side of the equation is not equal to $k_Z$ for the right side. If the zone has an end time, at the end time of the zone $u=0$, and we also know that the further in time solutions are classical.
