...
\begin{proof}
Consider\fxerror{The formula is wrong.}
\[ m=\sup_{f\in\prod_{a\in S}\setcond{f\in\SUPER(\mathbb{R})}{f=\lim_{x\to 0}f(x)=a}}\inf_{D\in\supfun{\SUPER(\mathbb{R})}\{0\}}f|_D \]
for $f\in S$. Then $m\in\SUPER(\mathbb{R})$ and $m\le a$ for each $a\in S$.
