...
\fxnote{Formulate the proof exactly.}Let $F$ be a supersingular function. Let $\mathcal{X}$ be an ultrafilter or, equilently, a sequence of points converging to a point $x$. $F\mathcal{X} = \lim_{\mathcal{X}\to x}f^{(n)}(x)$.

By the above shown interchange of limits,
\[ G_t F\mathcal{X} = \lim_{\mathcal{X}\to x}G_t f^{(n)}(x) = G_t\lim_{\mathcal{X}\to x}f^{(n)}(x). \]
That is, it is infinitely smoothing supersingular functions at each ultrafilter.
\end{proof}
