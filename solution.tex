...
\begin{enumerate}
\item ``$a\in\overline{Z_k}$ and $\forall\epsilon>0:\exists x\in\mathbb{R}:(\abs{a-x}<\epsilon\land f(x)=0)$'' ??
...
Really, the distance between $c(x)$ and $c(b)$\fxwarning{What is~$c$? Give full proof.} for $\forall p,q\in\mathbb{R}:pa\ne qb$ is
